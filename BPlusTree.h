...
template <class ElementType>
void BPlusTree<ElementType>::DropTree(Node pnode)
{ //删除整棵树
	if (!pnode)
		return;
	if (!pnode->isLeaf)
...
template <class ElementType>
void BPlusTree<ElementType>::WriteBack()
{ //将key和offset信息写回文件中
...
template <class ElementType>
offsetNumber BPlusTree<ElementType>::Search(ElementType key)
{ //在树中查找key并返回偏移量
	if (!root)
		return -1;
...
template <class ElementType>
bool BPlusTree<ElementType>::Insert(ElementType key, offsetNumber offset)
{ //根据偏移量插入key
...
template <class ElementType>
bool BPlusTree<ElementType>::Delete(ElementType key)
{ //从树中删除key
	NodeInfo<ElementType> newnode;
	if (!root)
	{ //树为空
	...
	else
	{
		...
		{
			if (newnode.index == 0 && leafhead != newnode.pNode)
			{ //key在分支中，且在当前结点左边
				int index = 0;
...
template <class ElementType>
bool BPlusTree<ElementType>::AfterInsert(Node pNode)
{ //插入key后的调整
	ElementType key;
	offsetNumber offset;
...
template <class ElementType>
ElementType BPlusTree<ElementType>::FindMin(Node pNode)
{ //查找最小key
	if (pNode->childs[0])
	{ //还有更小子树，则递归查找
...
template <class ElementType>
bool BPlusTree<ElementType>::AfterDelete(Node pNode)
{										   //删除key后调整
	...
	{ //叶结点
		...
		Node brother = NULL;
		if (index != 0)
		{ //最小key不在最左边，说明当前结点有左兄弟
			brother = parent->childs[index];
			if (brother->keycount > half_of_degree)
			{ //左兄弟key数足够，从左兄弟中搬一个key过来凑够key数
			...
			{ //左兄弟key数不够，将当前结点的key并入左兄弟，并删除当前结点
				for (int i = 0; i < pNode->keycount; i++)
				{
	...
	{ //不是叶结点，类似叶结点的处理，但不需要处理offset，而是要注意key转移时子结点的处理
		Node parent = pNode->parent;
		int index = 0;
...
template <class ElementType>
void BPlusTree<ElementType>::FindLeaf(Node pNode, ElementType key, NodeInfo<ElementType> &info)
{ //在叶结点中查找key，并将信息保存在info中
	int index = 0;
...
template <class ElementType>
void BPlusTree<ElementType>::printleaf()
{
	Node p = leafhead;
	while (p != NULL)
	{
